...
class Year:
    ...
    def NumberOfPowerplants(self):
        ...
        for plant in self.plant_types:
            ...
            string = string + "<br/>     {number} {type} plants".format(number=round(self.CalculateKW() / average),
                                                                     type=plant)
        return string
    ...
    def run(self):
        ...
        return "By {year} Australia will have a projected population of {pop}, to support this population Australia " \
               "will have to produce at least {kw} Billion Kwh per year. <br/>This is equivalent to:".format(
                year=self.year, pop=self.population, kw=kilowatt / 1000000000) + self.NumberOfPowerplants()
